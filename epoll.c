...
{
	ListeningData ld;
	if (try_malloc(sizeof(struct _listening_data), (void *)&ld) == -1)
	{
		return NULL;
	}
	ld->fd = fd;
...
{
	CTextData tclient;
	if (try_malloc(sizeof(struct _client_text_data), (void *)&tclient) == -1)
		return NULL;
	if (try_malloc(sizeof(char) * MAX_READ, (void *)&(tclient->buf)) == -1)
		return NULL;
	tclient->lenBuf = 0;
	return tclient;
}
...
{
	CBinData bclient;
	if (try_malloc(sizeof(struct _client_bin_data), (void *)&bclient) == -1)
		return NULL;
	bclient->state = STATE_COMMAND;
	return bclient;
}
